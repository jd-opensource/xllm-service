...
XllmRpcServiceImpl::~XllmRpcServiceImpl() { executor_->exited(); }

void XllmRpcServiceImpl::heartbeat(const proto::HeartbeatRequest* req) {
  executor_->handle_instance_heartbeat(req);
}

InstanceMetaInfo XllmRpcServiceImpl::get_instance_info(
    const std::string& instance_name) {
  return executor_->get_instance_info(instance_name);
}

std::vector<std::string> XllmRpcServiceImpl::get_static_decode_list(
    const std::string& instance_name) {
  return executor_->get_static_decode_list(instance_name);
}

std::vector<std::string> XllmRpcServiceImpl::get_static_prefill_list(
    const std::string& instance_name) {
  return executor_->get_static_prefill_list(instance_name);
}

bool XllmRpcServiceImpl::handle_generation(
    const llm::RequestOutput& request_output) {
  return executor_->handle_generation(request_output);
}

XllmRpcService::XllmRpcService(const Options& options, Executor* executor) {
  xllm_rpc_service_impl_ =
      std::make_unique<XllmRpcServiceImpl>(options, executor);
}
