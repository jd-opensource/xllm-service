...
#include "request/tokenizer/tokenizer_factory.h"
#include "scheduler/cache_aware_routing.h"
#include "scheduler/round_robin.h"
#include "scheduler/slo_aware_policy.h"
...
Executor::Executor(const Options& options) : options_(options) {
  ...
  auto handle_xservice = std::bind(&Executor::handle_xservice_watch,
                                   this,
                                   std::placeholders::_1,
                                   std::placeholders::_2);
  ...
  if (options.scheduling_policy() == "CAR") {
    scheduling_policy_ =
        std::make_unique<CacheAwareRouting>(instance_mgr_, global_kvcache_mgr_);
  } else if (options.scheduling_policy() == "SLO_AWARE") {
    scheduling_policy_ =
        std::make_unique<SloAwarePolicy>(options, instance_mgr_);
  } else {
    scheduling_policy_ = std::make_unique<RoundRobin>(instance_mgr_);
  }

  if (is_master_service_) {
    heartbeat_thread_ = std::make_unique<std::thread>(
        &Executor::update_master_service_heartbeat, this);
  } else {
    auto handle_master = std::bind(&Executor::handle_master_service_watch,
                                   this,
                                   std::placeholders::_1,
                                   std::placeholders::_2);
...
Executor::~Executor() { etcd_client_->stop_watch(); }

bool Executor::execute(std::shared_ptr<Request> request) {
  // apply chat template
  ...
  auto ret = scheduling_policy_->select_instances_pair(request);
...
std::shared_ptr<brpc::Channel> Executor::get_channel(
    const std::string& target_name) {
  return instance_mgr_->get_channel(target_name);
}

void Executor::update_master_service_heartbeat() {
  while (!exited_) {
...
bool Executor::register_current_service() {
...
void Executor::handle_instance_heartbeat(const proto::HeartbeatRequest* req) {
  if (exited_) {
    return;
  }
...
void Executor::handle_master_service_watch(const etcd::Response& response,
...
void Executor::handle_xservice_watch(const etcd::Response& response,
                                     const uint64_t& prefix_len) {
  if (exited_ || response.events().empty()) {
    return;
  }
...
InstanceMetaInfo Executor::get_instance_info(const std::string& instance_name) {
  return instance_mgr_->get_instance_info(instance_name);
}

std::vector<std::string> Executor::get_static_decode_list(
    const std::string& instance_name) {
  return instance_mgr_->get_static_decode_list(instance_name);
}

std::vector<std::string> Executor::get_static_prefill_list(
    const std::string& instance_name) {
  return instance_mgr_->get_static_prefill_list(instance_name);
}

Tokenizer* Executor::get_tls_tokenizer() {
  thread_local std::unique_ptr<Tokenizer> tls_tokenizer(tokenizer_->clone());
  return tls_tokenizer.get();
}

bool Executor::record_new_request(std::shared_ptr<ChatCallData> call_data,
...
bool Executor::record_new_request(std::shared_ptr<CompletionCallData> call_data,
                                  std::shared_ptr<Request> request) {
  {
    std::lock_guard<std::mutex> guard(request_mutex_);
    if (requests_.find(request->service_request_id) != requests_.end()) {
...
void Executor::finish_request(const std::string& service_request_id,
                              bool error) {
  {
    std::lock_guard<std::mutex> guard(request_mutex_);
...
void Executor::clear_requests_on_failed_instance(
    const std::string& instance_name,
    InstanceType type) {
...
bool Executor::handle_generation(const llm::RequestOutput& request_output) {
...
void Executor::update_request_metrics(std::shared_ptr<Request> request,
                                      bool finished_on_prefill_instance) {
...
void Executor::update_token_latency_metrics(std::shared_ptr<Request> request,
                                            bool finished_on_prefill_instance) {
