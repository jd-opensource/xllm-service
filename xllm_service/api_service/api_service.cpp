...
void handle_first_send_request(brpc::Controller* cntl,
  ...
  if (cntl->Failed()) {
    LOG(ERROR) << "Fail to send stream generation, " << cntl->ErrorText();
    call_data->finish_with_error(cntl->ErrorText());
    executor->finish_request(service_request_id, /*error*/ true);
    return;
  }
}
...
void XllmHttpServiceImpl::handle(std::shared_ptr<T> call_data,
  ...
  bool success = executor_->record_new_request(call_data, request);
  if (!success) {
  ...
  brpc::Channel* channel_ptr = executor_->get_channel(target_uri).get();
  // use stub
...
void XllmHttpServiceImpl::get_serving_models(
  ...
  if (!executor_->execute(service_request)) {
  ...
  brpc::Channel* channel_ptr =
      executor_->get_channel(service_request->routing.prefill_name).get();
